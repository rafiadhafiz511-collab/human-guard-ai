...
export default function Devices() {
  ...
  const loadDevices = async () => {
    ...
    try {
      ...
      setError(
        "Server থেকে ডিভাইস লোড করা সম্ভব হয়নি।"
      );
    } finally {
      setLoading(false);
    }
  ...
  const toggleDevice = async (device: Device) => {
    if (device.status !== "ONLINE" || device.state === "UNKNOWN") {
      return;
    }
      ...
      <div className="p-6 md:p-8">
        <div className="rounded-3xl border border-dashed border-white/10 bg-black/40 p-12 text-center">
          ...
          <p className="mt-2 text-xs text-white/40">
            ডিভাইসসমূহ দেখতে অনুগ্রহ করে একটি হোম নির্বাচন করুন।
          </p>
        </div>
      </div>
    );
                ...
                <div>
                  ...
                  <div className="mt-4 space-y-2 border-t border-white/5 pt-3 text-xs text-white/60">
                    <div className="flex justify-between gap-4">
                      <span>Location:</span>
                      <span className="font-medium text-white truncate max-w-[120px]" title={device.home_id || "Unassigned"}>
                        {device.home_id || "Unassigned"}
                      </span>
                    </div>
