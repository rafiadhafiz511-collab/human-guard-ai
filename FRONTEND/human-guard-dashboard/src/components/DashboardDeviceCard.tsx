...
export default function DashboardDeviceCard({
  ...
  const isPumpOn = device.state?.toUpperCase() === "ON";
  ...
  async function sendPumpCommand(
    ...
    if (
      (command === "PUMP_ON" && isPumpOn) ||
      (command === "PUMP_OFF" && !isPumpOn)
    ) {
      return;
    }
  ...
  return (
    ...
    >
      {/* TOP ROW: ICON, TITLE, QUICK SLIDER/STATE */}
      <div>
        <div className="flex items-center justify-between">
          ...
          <span
            className={`flex h-2.5 w-2.5 rounded-full ${
              isOnline ? "bg-emerald-400 shadow-[0_0_8px_#34d399]" : "bg-rose-500/50"
            }`}
          />
        </div>

        {/* CUSTOM AMBIENT PROGRESS BAR / RANGE */}
        <div className="mt-5">
      ...
      <div className="mt-6 flex items-center justify-between border-t border-white/5 pt-4">
        <span className="text-xs font-medium text-white/40">Power</span>

        <button
          type="button"
          disabled={loading || !isOnline}
          onClick={(e) => sendPumpCommand(isPumpOn ? "PUMP_OFF" : "PUMP_ON", e)}
          className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors duration-300 ${
            isPumpOn ? "bg-cyan-500" : "bg-white/10"
          }`}
        >
          <span
            className={`inline-block h-5 w-5 transform rounded-full bg-white shadow-md transition-transform duration-300 ${
              isPumpOn ? "translate-x-6" : "translate-x-1"
            }`}
          />
        </button>
      </div>
